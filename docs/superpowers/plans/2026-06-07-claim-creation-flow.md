...
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/selection/SelectionService.java` for completed selections that persist until consumed or cleared.
- Create `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/selection/DoubleCrouchClearService.java` for tick-window clear detection.
- Create `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/claim/ClaimIndex.java` for live lookup by chunk.
- Create `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/claim/ClaimCreationService.java` for validation and creation.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/tool/ClaimToolService.java` for reading and spending charges.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/storage/sql/SqlClaimRepository.java` for save/load methods.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/listener/ClaimToolListener.java` for clear gestures.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/listener/ProtectionListener.java` to use `ClaimIndex`.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/command/ClaimsCommand.java` for `create`, `cancel`, and `info`.
- Modify `havenclaims-plugin/src/main/java/com/invisiblespiders/havenclaims/plugin/HavenClaimsPlugin.java` for startup wiring.
- Modify `havenclaims-plugin/src/main/resources/config.yml` for claim name and selection clear settings.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.selection.SelectionServiceTest` and confirm the new tests fail because the API is missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.selection.DoubleCrouchClearServiceTest` and confirm it fails because the service is missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.claim.ClaimIndexTest` and confirm it fails because `ClaimIndex` is missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.storage.sql.SqlClaimRepositoryTest` and confirm it fails with `UnsupportedOperationException`.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.tool.ClaimToolServiceTest` and confirm the new APIs are missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.claim.ClaimCreationServiceTest` and confirm it fails because the service is missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.listener.ClaimToolListenerTest` and confirm the helper is missing.
...
- [ ] Run `./gradlew.bat :havenclaims-plugin:test --tests com.invisiblespiders.havenclaims.plugin.listener.ProtectionListenerTest` and confirm constructor/API failure.
- [ ] Update `ProtectionListener` to use `ClaimIndex` instead of an internal map.
- [ ] Wire `SqlClaimRepository`, `ClaimIndex`, `ClaimCreationService`, `DoubleCrouchClearService`, listeners, and command in `HavenClaimsPlugin`.
...
- [ ] Confirm `havenclaims-plugin/build/libs/HavenClaims-1.0.0-SNAPSHOT.jar` exists.
- [ ] Run `git status -sb` and confirm only local artifacts are untracked.
- [ ] Push `feat/claim-creation-flow`.
- [ ] Open a PR against `master`.
- [ ] Watch GitHub Actions and confirm the `HavenClaims` artifact uploads.
